...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  saveshibperblock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, saveshibPerBlock] =
    pid || pid === 0
  ...
  const SaveshibPerBlock = saveshibPerBlock ? new BigNumber(saveshibPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    saveshibperblock: SaveshibPerBlock.toJSON(),
    canharvest
  }
}
